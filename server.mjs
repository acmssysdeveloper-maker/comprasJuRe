...
const server=http.createServer(async(req,res)=>{
 try{
  ...
  if(req.method==='GET'&&u.pathname==='/api/status')return send(res,200,{ok:true,version:'2.7.1',gemini:!!KEY,ocrSpace:!!OCR_KEY,consultadanfe:!!DANFE_URL,model:MODEL});
...
server.listen(PORT,'127.0.0.1',()=>console.log(`Compras da JuRe 2.7.1 — http://localhost:${PORT} — Gemini ${KEY?'ATIVO':'sem chave'} — OCR.space ${OCR_KEY?'ATIVO':'sem chave'} — Consulta DANFE ${DANFE_URL?'ATIVA':'sem URL'}`));
