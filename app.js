...
*/
const APP_VERSION="2.6.5", DB_NAME="compras-jure-db", DB_VERSION=3;
...
function extractItemRowsFromLines(lines){
 ...
 for(let i=0;i<lines.length;i++){
   ...
   const qm=window.match(/(?:qtde|qtd|quantidade)\s*(?:[:.]\s*)+(\d+(?:[\.,]\d+)?)\s*(?:un|und|unid)?/i)||window.match(/^\s*(\d+(?:[\.,]\d+)?)\s+(?:UN|UND|UNID|KG|L|ML|G)\b/i);const qty=qm?receiptNum(qm[1]):1;
   const um=window.match(/\b(?:UN|UND|UNID|KG|L|ML|G)\b/i);const unit=um?normalizeUnit(um[0]):'un';
   const money=[];
   ...
   if(m.name&&total>0)out.push({barcode:m.barcode,name:m.name,quantity:qty,unit,unitPrice,total,discount,brand:'',category:inferProductCategory(m.name),pack:guessPack(m.name),needsReview:true,confidence:best.err<=.03?.90:.58,sourceLine:i,gtinValid:isValidGtin(m.barcode)});
 }
 return out;
}
...
function validateReceiptData(d){
 ...
 const diff=Math.abs(lineSum-total);if(diff>0.05)return {ok:false,message:`As linhas somam ${money(lineSum)}, mas o total identificado é ${money(total)}. Diferença de ${money(diff)}. A compra precisa ser conferida.`};
 if(d.audit?.status==='BLOCK')return {ok:false,message:d.audit.issues?.[0]||'A auditoria fiscal bloqueou a publicação desta compra.'};
 if(d.audit?.duplicate&&d.fiscalAutoLaunch)return {ok:false,message:'A auditoria identificou uma compra já registrada. O lançamento automático foi bloqueado para evitar duplicidade.'};
 if((d.items||[]).some(i=>i.matchStatus==='ambiguous'))return {ok:false,message:'Há produtos com associação ambígua no catálogo. O sistema não publicará a ocorrência até que a associação seja confirmada.'};
...
async function auditAndContextualizeReceipt(receipt){
 ...
 out.audit=window.ReceiptAuditor?.auditReceipt?window.ReceiptAuditor.auditReceipt(out,d.purchases||[]):null;out.auditReady=!!out.audit;
 const exact=(out.items||[]).filter(i=>i.matchStatus==='exact').length,contextual=(out.items||[]).filter(i=>i.matchStatus==='contextual').length,ambiguous=(out.items||[]).filter(i=>i.matchStatus==='ambiguous').length;
 out.catalogMatch={exact,contextual,ambiguous,new:(out.items||[]).filter(i=>i.matchStatus==='new').length,total:out.items?.length||0};
 out.items=(out.items||[]).map(i=>({...i,confidence:Math.min(1,Math.max(num(i.confidence)||0,i.matchConfidence||0)),needsReview:!!(i.needsReview||i.matchStatus==='ambiguous'||(i.matchConfidence||0)<.74)}));return out;
}
function renderReceiptAudit(d){
 const a=d?.audit;if(!a)return '';const cls=a.status==='PASS'?'good':a.status==='REVIEW'?'warn':'danger';const title=a.status==='PASS'?'Auditoria aprovada':a.status==='REVIEW'?'Auditoria requer conferência':'Auditoria bloqueou a publicação';
 const lines=[`Matemática: linhas líquidas ${money(a.math.netSum)} · total ${money(a.math.total)} · diferença ${money(Math.abs(a.math.netDelta))}`,`Itens auditados: ${a.itemAudits.length}${d.itemCountDeclared?` · declarados: ${d.itemCountDeclared}`:''}`,`Catálogo: ${d.catalogMatch?.exact||0} exatos · ${d.catalogMatch?.contextual||0} contextuais · ${d.catalogMatch?.ambiguous||0} ambíguos · ${d.catalogMatch?.new||0} novos`];
 if(a.duplicate)lines.push(`Possível duplicidade: ${a.duplicate.market||a.duplicate.marketName||'estabelecimento'} · ${fmtDate(a.duplicate.date)} ${a.duplicate.time||''} · ${money(a.duplicate.total)}`);
 const errs=(a.issues||[]).slice(0,3).concat((a.warnings||[]).slice(0,3));return `<div class="audit-block ${cls}"><b>${title}</b><div>${lines.map(esc).join('<br>')}</div>${errs.length?`<small>${errs.map(esc).join(' · ')}</small>`:''}</div>`;
}
