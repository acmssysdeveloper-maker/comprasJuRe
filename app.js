...
*/
const APP_VERSION="2.6.4", DB_NAME="compras-jure-db", DB_VERSION=3;
...
async function seedIfEmpty(){
  ...
  await put("meta",{id:"meta",schemaVersion:2,appVersion:APP_VERSION,seededAt:todayISO()});
}
async function migrate(){
  ...
  if(from<3){
    ...
    for(const store of ["products","markets","purchases"]){
    ...
    await put("meta",{...m,schemaVersion:Math.max(3,Number(m.schemaVersion||3)),appVersion:APP_VERSION});
  }
}
...
async function renderLists(){
  const d=await data();const lists=d.lists.filter(l=>l.active!==false).sort((a,b)=>(b.createdAt||"").localeCompare(a.createdAt||""));
  $("#listsArea").innerHTML=lists.map(l=>`<div class="stack-card"><div class="purchase-top"><div><h3 style="margin:0">${l.name}</h3><div class="muted">${new Date(l.createdAt).toLocaleString("pt-BR")} · ${l.items?.length||0} produtos</div></div><div class="purchase-actions"><button class="primary" onclick="openList('${l.id}')">Abrir lista</button><button class="ghost" onclick="archiveList('${l.id}')">Arquivar</button></div></div><div style="display:flex;gap:7px;flex-wrap:wrap;margin-top:10px">${(l.items||[]).slice(0,8).map(it=>`<span class="badge">${it.name} · ${it.qty||1}</span>`).join("")}</div></div>`).join("")||"<div class='panel'>Você ainda não criou uma lista.</div>"
}
$("#newListBtn").onclick=()=>newListModal();
function newListModal(){const body=`<div class="form-grid"><label>Nome da lista<input id="listName" value="Compra do mês"></label></div><h4>Selecione os produtos</h4><div style="max-height:420px;overflow:auto">${[...CURRENT_DATA.products||[]].filter(p=>p.active!==false).map(p=>`<label class="check" style="display:block;padding:8px 0;border-top:1px solid #eee"><input type="checkbox" class="list-prod" value="${p.id}"> ${p.name} <span class="muted">· ${p.pack||""}</span></label>`).join("")}</div><div class="button-row" style="margin-top:12px"><button class="primary" id="createList">Criar lista</button><button class="ghost" onclick="closeModal()">Cancelar</button></div>`;openModal("Nova lista",body);$("#createList").onclick=async()=>{const ids=$$(".list-prod:checked").map(x=>x.value),items=ids.map(id=>{const p=CURRENT_DATA.products.find(x=>x.id===id);return {productId:id,name:p.name,qty:1,checked:false}});if(!items.length)return toast("Selecione pelo menos um produto.");const l={id:uid("list"),name:$("#listName").value.trim()||"Nova lista",createdAt:todayISO(),active:true,items};await put("lists",l);await audit("Nova lista criada.","list",l.id,"create");closeModal();await renderLists();toast("Lista criada.")}}
window.openList=async id=>{const l=await getOne("lists",id);openModal(l.name,`<div>${(l.items||[]).map((it,idx)=>`<label class="check" style="display:block;padding:10px 0;border-top:1px solid #eee"><input type="checkbox" ${it.checked?"checked":""} data-listidx="${idx}"> ${it.name} <span class="muted">quantidade <input style="width:70px;margin-left:4px" data-qtyidx="${idx}" value="${it.qty||1}"></span></label>`).join("")}</div><div class="button-row" style="margin-top:14px"><button class="primary" id="saveList">Salvar lista</button><button class="ghost" id="closeList">Fechar</button></div>`);$("#closeList").onclick=closeModal;$("#saveList").onclick=async()=>{const items=[...(l.items||[])];items.forEach((it,i)=>{const c=$(`[data-listidx="${i}"]`);const q=$(`[data-qtyidx="${i}"]`);it.checked=c?.checked||false;it.qty=Math.max(1,num(q?.value)||1)});await put("lists",{...l,items,updatedAt:todayISO()});await audit("Lista atualizada.","list",id);closeModal();await renderLists();toast("Lista salva.")}};
window.archiveList=async id=>{const l=await getOne("lists",id);await put("lists",{...l,active:false,archivedAt:todayISO()});await audit("Lista arquivada.","list",id,"archive");await renderLists();toast("Lista arquivada.")};

let CURRENT_DATA={};
async function refreshData(){CURRENT_DATA=await data();return CURRENT_DATA}

async function renderProducts(){
 const d=await refreshData(),q=normalize($("#productSearch")?.value||""),cat=$("#productCategoryFilter")?.value||"",health=$("#healthFilter")?.value||"";
...
async function renderMarkets(){
 const d=await refreshData(),active=d.purchases.filter(p=>p.active!==false),map={};for(const p of active){map[p.marketId]??={name:p.marketName,cnpj:p.cnpj,city:p.city,total:0,count:0};map[p.marketId].total+=num(p.total);map[p.marketId].count++}
 $("#marketCards").innerHTML=Object.values(map).sort((a,b)=>b.total-a.total).map(m=>`<div class="market-card"><h3>${m.name}</h3><div class="muted">${m.city||""} · ${m.cnpj||"CNPJ não informado"}</div><strong>${money(m.total)}</strong><p class="muted">${m.count} compra(s) registrada(s). A comparação por produto ficará mais precisa quando houver o mesmo item em mais de um mercado.</p></div>`).join("")||"<div class='panel'>Nenhum mercado.</div>"
}
async function renderConsumption(){
 const d=await refreshData(),sel=$("#consumptionProduct"),products=d.products.filter(p=>p.active!==false),current=sel.value;
...
async function renderEconomy(){
 const d=await refreshData(),active=d.purchases.filter(p=>p.active!==false),confirmed=active.reduce((s,p)=>s+num(p.discountTotal),0);let opp=0,above=0,excess=0,rows=[];
...
async function renderReport(){
 const d=await refreshData(),active=d.purchases.filter(p=>p.active!==false),total=active.reduce((s,p)=>s+num(p.total),0),items=d.purchaseItems.filter(i=>active.some(p=>p.id===i.purchaseId));const categories={};for(const i of items){const p=d.products.find(x=>x.id===i.productId);categories[p?.category||"Outros"]=(categories[p?.category||"Outros"]||0)+num(i.totalAfterDiscount??i.lineTotal)}
 $("#reportContent").innerHTML=`<h2>Compras da JuRe</h2><p>Relatório gerado em ${new Date().toLocaleString("pt-BR")}</p><div class="report-grid"><div class="card"><span>Total</span><strong>${money(total)}</strong></div><div class="card"><span>Compras</span><strong>${active.length}</strong></div><div class="card"><span>Produtos</span><strong>${d.products.filter(p=>p.active!==false).length}</strong></div><div class="card"><span>Itens/ocorrências</span><strong>${items.length}</strong></div></div><h3>Por categoria</h3><ul>${Object.entries(categories).sort((a,b)=>b[1]-a[1]).map(([k,v])=>`<li>${k}: ${money(v)}</li>`).join("")}</ul><h3>Integridade</h3><p>O relatório preserva a data original de cada compra. Correções alteram o registro atual, mas ficam registradas em auditoria; exclusões são arquivamentos lógicos.</p>`
}
...
if(restoreBackupBtn && restoreBackupFile){
 ...
 try{
  ...
  if(payload.data.meta) await put("meta",{...payload.data.meta,schemaVersion:3,appVersion:APP_VERSION});
  else await put("meta",{id:"meta",schemaVersion:3,appVersion:APP_VERSION,restoredAt:todayISO()});
